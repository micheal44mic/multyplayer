...
import { delay, runRafCountdown, runRewardedSdk } from './rewarded_ad.js';
...
const FALLBACK_REWARD_SECONDS = 0;
const REWARD_GRANTED_DELAY_MS = 0;
const SIZE_MAX = 2000;
...
export class PresetsUI {
  ...
  async _showRewardedAd(preset) {
    ...
    const handled = await runRewardedSdk({
      payload: {
        placement: 'premium_brush_unlock',
        brushId: preset.id,
        categoryId: preset.categoryId,
      },
      delayMs: REWARD_GRANTED_DELAY_MS,
      onStart: () => { this._adStatus.textContent = 'Playing fullscreen rewarded ad...'; },
      onGranted: () => {
    ...
    await runRafCountdown({
      seconds: FALLBACK_REWARD_SECONDS,
      onComplete: () => {
        this._adStatus.textContent = 'Ad complete';
        this._adMeter.style.transform = 'scaleX(1)';
      },
      onTick: ({ progress, remaining }) => {
        if (this._adStatus) this._adStatus.textContent = `Ad completes in ${remaining}s`;
        if (this._adMeter) this._adMeter.style.transform = `scaleX(${progress})`;
      },
    });
    this._adStatus.textContent = 'Reward granted';
    await delay(REWARD_GRANTED_DELAY_MS);
  }
}
