...
import { makeRasterLayer, makeTextLayer, makeSvgLayer, bumpLayerIds, BLEND_MODES } from './layers.js';
import { CHUNK_BYTES, isChunkBlank } from './store.js';
 ...
 */
/** @typedef {{id: string, snapshot: any}} ProjectRecord */
...
function snapshotLayer(layer) {
  ...
  if (layer.kind === 'raster') {
    ...
    out.svgItem = plainClone(layer.svgItem);
    out.ver = layer.ver || 0;
  }
  return out;
}
...
export function makeProjectSnapshot(app, name) {
  ...
  const snapshot = {
    ...
    spaceNodes: app.ui?.spaceNodes?.serialize() || [],
  };
  return snapshot;
}
...
export function projectStats(snapshot) {
  ...
  // i PNG dei nodi Spaces pesano nello snapshot quanto i chunk raster
  let nodeBytes = 0;
  for (const n of snapshot.spaceNodes || []) {
    for (const v of n.variants || []) nodeBytes += Math.floor(String(v).length * 0.75);
  }
  return {
    boards: (snapshot.boards || []).length,
    layers,
    chunks,
    spaceNodes: (snapshot.spaceNodes || []).length,
    rawBytes: chunks * CHUNK_BYTES + nodeBytes,
  };
}
...
export function applyProjectSnapshot(app, snapshot) {
  ...
  for (const sb of s.boards) {
    ...
    for (const sl of sb.layers || []) {
      ...
      if (sl.kind === 'text') {
        ...
        layer = makeSvgLayer(safeName(sl.name), plainClone(sl.svgItem || {}));
        layer.ver = Number(sl.ver) || 0;
        layer.svgDirty = true;
      } else {
  ...
  app.ui.spaceNodes.restore(Array.isArray(s.spaceNodes) ? s.spaceNodes : null);
  app.undoMgr.clear();
  app.boards.bump();
  app.planes.invalidate();
  app.ui.layersUI.open(false);
  app.ui.textUI.open(false);
  app.ui.svgUI.open(false);
...
/** @param {string} id @param {any} snapshot @param {number} [createdAt] @returns {Promise<ProjectMeta>} */
...
/** @param {string} id @returns {Promise<ProjectMeta|null>} */
...
export class ProjectHub {
  ...
  async saveNow(manual = false) {
    return this._save({ manual, refreshList: true, quiet: false });
  }

  async saveInBackground() {
    return this._save({ manual: false, refreshList: this.isOpen, quiet: true });
  }

  /** @param {{manual: boolean, refreshList: boolean, quiet: boolean}} options */
  async _save(options) {
    const { manual, refreshList, quiet } = options;
    ...
    try {
      ...
      if (refreshList) await this.refreshList();
      if (!quiet || this.isOpen) this.setStatus(`Salvato nella cartella lavori · ${fmtDate(meta.updatedAt)}`);
      ...
      if (!quiet || this.isOpen) this.setStatus(err instanceof Error ? err.message : 'Salvataggio nella cartella lavori fallito.');
      return false;
    } finally {
      this._saving = false;
    }
