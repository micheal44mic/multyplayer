...
const BRUSH_DEFAULTS = (() => {
  const { color, tool, blurSize, blurStrength, blurOpacity, blurSoftness, ...rest } = brush;
  return rest;
})();
...
export class UI {
  ...
  // ---- slider rapidi (touch): dimensione, opacità e morbidezza sul bordo sinistro ----
  // Visibili solo su pointer coarse (CSS): scrivono brush.size/opacity o i
  // corrispettivi del blur, e la morbidezza appare solo col tool Sfoca.
  ...
  _buildSideSliders() {
    ...
    const showPreview = (track, title, t) => {
      ...
      const size = clamp(brush.tool === 'blur' ? brush.blurSize : brush.size, 1, SIZE_MAX);
      const sizeT = clamp(Math.log(size) / Math.log(SIZE_MAX), 0, 1);
      const mark = Math.round(5 + Math.pow(sizeT, 0.82) * 76);
      const roundness = brush.tool === 'blur' ? 1 : clamp(brush.roundness || 1, 0.05, 1);
      ...
      previewMark.style.transform = `rotate(${brush.tool === 'blur' ? 0 : brush.angle || 0}deg)`;
    ...
    /** @type {(() => void)[]} */
    this._ssSync = [
      () => wrap.classList.toggle('has-softness', brush.tool === 'blur'),
      mk('Size',
        () => Math.log(clamp(brush.tool === 'blur' ? brush.blurSize : brush.size, 1, SIZE_MAX)) / lnMax,
        (t) => {
      ...
      mk('Opacity',
        () => brush.tool === 'blur' ? (brush.blurOpacity ?? 1) : brush.opacity,
        (t) => {
      ...
      mk('Softness',
        () => brush.blurSoftness,
        (t) => { brush.blurSoftness = Math.round(t * 100) / 100; },
        () => brush.tool === 'blur'),
    ];
  ...
  _buildBlurPopup() {
    ...
    close.title = 'Close';
    close.textContent = '✕';
    close.addEventListener('click', () => this.toggleBlurPopup(false));
    head.append(title, close);

    const body = document.createElement('div');
    body.className = 'blur-body';
    for (const row of BLUR_ROWS) body.appendChild(this._buildBlurSlider(row));
    ...
    document.addEventListener('pointerdown', (e) => {
      ...
      const btn = document.getElementById('tool-blur');
      if (t && (panel.contains(t) || btn.contains(t))) return;
      this.toggleBlurPopup(false);
    });
    window.addEventListener('resize', () => {
      if (panel.classList.contains('open')) this._positionBlurPopup();
    });
  }

  /** @param {RowDef} def */
  _buildBlurSlider(def) {
    const row = document.createElement('div');
    row.className = 'p-row';
    const head = document.createElement('div');
    head.className = 'p-row-head';
  ...
  _positionBlurPopup() {
    ...
    const h = panel.offsetHeight || 190;
  ...
  _bindToolbar() {
    ...
    for (const [id, kind] of [
      ['sel-kind-color', 'color'],
      ['sel-kind-lasso', 'lasso'],
      ['sel-kind-polygon', 'polygon'],
    ]) {
    ...
    for (const [id, op] of [
      ['sel-op-replace', 'replace'],
      ['sel-op-add', 'add'],
      ['sel-op-subtract', 'subtract'],
    ]) {
      document.getElementById(id).addEventListener('click', () => {
    ...
    document.getElementById('sel-finish').addEventListener('click', () => app.finishPolygonLasso());
    ...
    document.getElementById('btn-mockup').addEventListener('click', () => this.mockups.toggle());
  ...
  setTool(tool) {
    ...
    for (const [id, t] of [['tool-brush', 'brush'], ['tool-eraser', 'eraser'],
      ['tool-blur', 'blur'], ['tool-select', 'select'], ['tool-move', 'move'], ['tool-pan', 'pan']]) {
  ...
  _bindKeys() {
    ...
    window.addEventListener('keydown', (e) => {
      ...
      else if (k === 'escape') { app.cancelLasso(); app.selection.clear(); app.transform.cancel(); app.fx.escape(); app.layerStyle.escape(); if (app.fillUI) app.fillUI.dismiss(); this.toggleStudio(false); this.toggleBlurPopup(false); this.textUI.open(false); this.layersUI.open(false); this.presetsUI.open(false); this.mockups.open(false); }
  ...
  updateCursor(input, camera) {
    ...
    if (!show) return;
    const d = Math.max(4, (brush.tool === 'blur' ? brush.blurSize : brush.size) * camera.zoom);
    el.style.width = d + 'px';
