// Preview del pennello (Brush Studio): un tratto a S renderizzato con la
// STESSA pipeline del canvas (StrokeEngine -> Rasterizer JS) su un mini
// ChunkStore dedicato. Seed fisso: trascinare uno slider non fa "ballare"
// scatter e jitter tra un re-render e l'altro.
//
...
export class BrushPreview {
  ...
  render() {
    ...
    engine.begin(px(0), py(0), 1, { ...brush, size: sizePx, tool: 'brush' }, SEED);
    this.raster.beginStroke(engine.snap);
    for (let i = 1; i <= POINTS; i++) {
      engine.move(px(i / POINTS), py(i / POINTS), 1);
    }
    engine.end(x1, py(1), 1);
