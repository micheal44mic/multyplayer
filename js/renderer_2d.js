...
import { bevelEmbossBuffer } from './layer_styles.js';
...
/** @param {TransformFrameSet} transform @param {number} layerId @returns {import('./renderer_gl.js').TransformFrame|null} */
...
export class Canvas2DRenderer {
   ...
   * textQuads/svgQuads: i vettori NON in editing stanno nella pila e si
    ...
    for (let i = 0; i < layers.length; i++) {
      ...
      if (layer.kind === 'svg') {
        if (svgQuads) {
          ...
          if (q) this._drawVectorQuad(ctx, layer, q);
        }
        continue;
      }
      ...
      this._drawLayerInto(ctx, layer, activeId, strokeStore, strokeOpacity,
        eraserLive, transformForLayer(transform, layer.id), fx, cx0, cy0, cx1, cy1);
  ...
  _drawTextQuad(ctx, layer, q) {
    this._drawVectorQuad(ctx, layer, q);
  }

  // Quad di un livello vettoriale cotto: stesso path per testo e SVG.
  /** @param {CanvasRenderingContext2D} ctx @param {Layer} layer @param {{canvas: HTMLCanvasElement, x: number, y: number, w: number, h: number}} q */
  _drawVectorQuad(ctx, layer, q) {
  ...
  // quando i parametri cambiano. Gaussiana usa ctx.filter; halftone/bevel
  // usano il fallback CPU sui pixel premultiplied; gli altri effetti qui
  // restano nitidi in ANTEPRIMA — il commit CPU è comunque corretto.
  ...
  _ensureFxCanvas(fx) {
    ...
    const key = `${fx.kind}|${fx.sigma}|${fx.radius}|${fx.spacing}|${fx.angle}|${fx.colorMix}|` +
      `${fx.strokeW}|${fx.strokePos}|${fx.strokeR}|${fx.strokeG}|${fx.strokeB}|` +
      `${fx.bevel ? JSON.stringify(fx.bevel) : ''}|${fx.x}|${fx.y}`;
    ...
    if (fx.kind === 'halftone') {
      ...
      const data = snapshotRect(fx.store, fx.x, fx.y, fx.w, fx.h);
      bevelEmbossBuffer(data, fx.w, fx.h, fx.bevel);
      this._putPremul(cnv, data, fx.w, fx.h);
    } else {
