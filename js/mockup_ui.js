...
const CATEGORIES = [
  { id: 'bodyTee', label: 'Tee', items: MOCKUP_ASSETS.bodyTee },
  { id: 'bodyHoodie', label: 'Body hoodie', items: MOCKUP_ASSETS.bodyHoodie },
  { id: 'hood', label: 'Hood', items: MOCKUP_ASSETS.hood },
  { id: 'collar', label: 'Collar', items: MOCKUP_ASSETS.collar },
  { id: 'pocket', label: 'Pocket', items: MOCKUP_ASSETS.pocket },
];
...
export class MockupUI {
  ...
  _createAdGate() {
    ...
    const cancel = gate.querySelector('.mu-ad-cancel');
    const cont = gate.querySelector('.mu-ad-continue');
    ...
    return {
      el: gate,
      name: gate.querySelector('[data-ad-name]'),
      status: gate.querySelector('[data-ad-status]'),
      progress: gate.querySelector('[data-ad-progress]'),
      cancel,
      cont,
    };
  }
  ...
  _startAdGatePlayback() {
    ...
    const tick = () => {
      ...
      if (remaining === 0 && this._adTimer) {
        window.clearInterval(this._adTimer);
        this._adTimer = null;
        window.setTimeout(() => this._finishAdGate(true), 350);
      }
    };

    tick();
    this._adTimer = window.setInterval(tick, 200);
  }
 ...
 * @property {string} id
 * @property {'hood'|'collar'|'pocket'|'bodyHoodie'|'bodyTee'} kind
...
/**
 * @param {'hood'|'collar'|'pocket'|'bodyHoodie'|'bodyTee'} kind
 * @param {string} label
 * @param {number} count
 * @returns {MockupItem[]}
 */
function makeItems(kind, label, count) {
  const out = [];
  for (let i = 0; i < count; i++) {
    const n = Math.floor(i / 2) + 1;
    const view = i % 2 === 0 ? 'back' : 'front';
    const suffix = view === 'back' ? ' back' : '';
    out.push({
      id: `${kind}-${i + 1}`,
      kind,
      variant: i,
