...
import { CHUNK_SHIFT, contentBBox, translateStore, transformStore } from './store.js';
...
export class TransformTool {
  ...
  constructor(app) {
    ...
    svg.append(this.gridEl, this.handleEl, this.edgeEl, this.spokeEl);
    ...
    this._tabAffine = /** @type {HTMLButtonElement} */ (document.getElementById('tf-tab-affine'));
    this._tabPersp = /** @type {HTMLButtonElement} */ (document.getElementById('tf-tab-persp'));
    this._tabWarp = /** @type {HTMLButtonElement} */ (document.getElementById('tf-tab-warp'));
    this._tabPuppet = /** @type {HTMLButtonElement} */ (document.getElementById('tf-tab-puppet'));
    this._gridWrap = document.getElementById('tf-grid');
    /** @type {HTMLButtonElement[]} */
    this._gridBtns = Array.from(this._gridWrap.querySelectorAll('.tf-gbtn'));
    this._tabAffine.addEventListener('click', () => this.setMode('affine'));
    this._tabPersp.addEventListener('click', () => this.setMode('persp'));
    this._tabWarp.addEventListener('click', () => this.setMode('warp'));
    this._tabPuppet.addEventListener('click', () => this.setMode('puppet'));
    ...
    this._ppActMove = /** @type {HTMLButtonElement} */ (document.getElementById('pp-act-move'));
    this._ppActRot = /** @type {HTMLButtonElement} */ (document.getElementById('pp-act-rot'));
    ...
    this._ppActMove.addEventListener('click', () => this._setPupAction('move'));
    this._ppActRot.addEventListener('click', () => this._setPupAction('rotate'));
  ...
  sync(cam) {
    ...
    const isText = !!cur && cur.kind === 'text';
    if (isText && this.mode !== 'affine') this.setMode('affine');
    if (this._tabWarp.disabled !== isText) this._tabWarp.disabled = isText;
    if (this._tabPersp.disabled !== isText) this._tabPersp.disabled = isText;
    if (this._tabPuppet.disabled !== isText) this._tabPuppet.disabled = isText;
  ...
  setMode(m) {
    ...
    const s = this._session;
    if (m !== 'affine' && s && s.kind === 'text') return; // prima si rasterizza
  ...
  _syncOpts() {
    this._tabAffine.classList.toggle('active', this.mode === 'affine');
    this._tabPersp.classList.toggle('active', this.mode === 'persp');
    this._tabWarp.classList.toggle('active', this.mode === 'warp');
    this._tabPuppet.classList.toggle('active', this.mode === 'puppet');
  ...
  _syncPupAction() {
    const move = this._pupAction === 'move';
    this._ppActMove.classList.toggle('active', move);
    this._ppActRot.classList.toggle('active', !move);
    this._ppActMove.setAttribute('aria-pressed', move ? 'true' : 'false');
    this._ppActRot.setAttribute('aria-pressed', move ? 'false' : 'true');
  }
