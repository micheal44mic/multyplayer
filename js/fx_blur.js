...
function particleNoise(px, py, size, roughness, seed) {
  ...
  return clamp(clump * (1 - microMix) + micro * microMix, -1, 1);
}
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++, o += 4) {
      ...
      for (let ch = 0; ch < 3; ch++) {
        ...
        data[o + ch] = clamp(data[o + ch] + n * amt, 0, a);
      }
    }
  }
}
...
function filmGrainNoise(px, py, size, roughness, seed) {
  ...
  const rgh = clamp(roughness, 0, 1);
  const sizeMix = clamp((safeSize - 1) / 14, 0, 1);
...
export function grainBuffer(data, w, h, amount, size, roughness, seed) {
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++, o += 4) {
      ...
      const mid = clamp(4 * lum * (1 - lum), 0, 1);
      const tone = 0.16 + 0.84 * Math.pow(mid, 0.72);
      const k = n * amount * tone * a;
      data[o] = clamp(data[o] + k, 0, a);
      data[o + 1] = clamp(data[o + 1] + k, 0, a);
      data[o + 2] = clamp(data[o + 2] + k, 0, a);
    }
  }
}
...
/** @param {{c:number,s:number,r:number,cell:number}} sc @param {number} px @param {number} py @param {number} amount */
...
// Traccia (Stile livello) IN PLACE: contorno della forma (alpha>0) di
...
export function strokeBuffer(data, w, h, sw, pos, r, g, b) {
  ...
  for (let i = 0, o = 0; i < n; i++, o += 4) {
    ...
    // del bordo (dIn < 2, alpha > 0): il residuo 1-a va saturato, sennò
