...
import { makeRasterLayer, makeTextLayer, makeSvgLayer, duplicateLayer, bumpLayerIds } from './layers.js';
...
import { freeSvgPlane } from './svg_layer.js';
...
const BRUSH_FIELDS = ['size', 'opacity', 'hardness', 'smoothing', 'stabilizationMode',
  'pressureSize', 'pressureCurveX', 'pressureCurveY', 'spacing',
  ...
  'aquaEnabled', 'aquaColorMix', 'aquaWetness', 'aquaLighten',
...
export class Collab {
  ...
  constructor(app) {
    ...
    /** @type {Map<number, string>} */ this._sshadow = new Map();
    this._watchT = 0;
    ...
    this._muteSform = false;   // pushStruct 'svgform' da apply remoto: no ribroadcast
  ...
  _begin() {
    ...
    for (const b of a.boards.boards) {
      for (const l of b.mgr.layers) {
        ...
        if (l.kind === 'svg') this._sshadow.set(l.id, this._ssig(l));
      }
    }
    this._jobs.length = 0;
    this._rs.clear();
    ...
    switch (m.t) {
      case 'sb': {
        ...
        this._rs.set(uid, {
          ...
          br: m.br, x: m.x, y: m.y, p: m.p, tm: m.tm, dp: !!m.dp, ev: [],
        });
  ...
  /** @param {import('./boards.js').Board} board @param {number} layerId @param {number} x @param {number} y @param {number} p @param {number} t @param {boolean} [direct] */
  strokeBegin(board, layerId, x, y, p, t, direct = false) {
  ...
  _applyStroke(s) {
    ...
    a.queue.patternTile = s.pt && s.pt.w > 0 && s.pt.h > 0
      ...
      : null;
    a.engine.begin(s.x, s.y, s.p, s.tm, rb, s.seed, s.sc, !!s.dp);
    a.raster.beginStroke(a.engine.snap, a._strokeClip, null, layer.store);
    a.strokeLive = true;
  ...
  _brushFor(d) {
    ...
    if (rb.stabilizationMode !== 'rope') rb.stabilizationMode = 'smart';
  ...
  _applyOp(o, uid) {
    ...
    switch (o.k) {
      ...
      case 'lmerge': return this._applyLayerMerge(o, uid ?? -1);
      ...
      // trasformazione affine di un livello SVG: matrice finale assoluta
      case 'sform': {
        const board = a.boards.boardOfLayer(o.l);
        const layer = board && board.mgr.byId(o.l);
        if (!board || !layer || layer.kind !== 'svg' || !layer.svgItem) return;
        const m = Array.isArray(o.m) && o.m.length === 6
          ? /** @type {[number, number, number, number, number, number]} */ (o.m.map((v) => Number(v)))
          : null;
        if (!m || m.some((v) => !Number.isFinite(v))) return;
        const m0 = /** @type {[number, number, number, number, number, number]} */ (layer.svgItem.m.slice());
        if (!a._undoHost().setSvgTransform(o.l, m)) return;
        this._muteSform = true;
        try {
          a.undoMgr.pushStruct(/** @type {any} */ ({
            op: 'svgform', layerId: o.l, boardId: board.id,
            tm0: m0, tm1: m.slice(),
          }));
        } finally {
          this._muteSform = false;
        }
        if (o.cid) this._tagNewEntry(o.cid);
        this._sshadow.set(o.l, this._ssig(layer)); // niente eco sset dal watcher
        a.planes.invalidate();
        a.ui.layersUI.scheduleThumbs();
        this._clearRemoteTransform(uid ?? -1, o.l);
        return;
      ...
      case 'srast': return this._applySvgRaster(o, uid ?? -1);
      case 'ladd': {
        ...
        if (layer.kind === 'svg') this._sshadow.set(layer.id, this._ssig(layer));
        a.planes.invalidate();
        a.ui.layersUI.sync();
        a.ui.layersUI.scheduleThumbs();
        return;
      ...
      // contenuto di un livello SVG cambiato (colori/opacita interne):
      // stato completo del vettore, ultimo-scrittore-vince.
      case 'sset': {
        const board = a.boards.boardOfLayer(o.id);
        const layer = board && board.mgr.byId(o.id);
        if (!board || !layer || layer.kind !== 'svg') return;
        layer.svgItem = structuredClone(o.svgItem);
        layer.svgDirty = true;
        layer.ver = (layer.ver || 0) + 1;
        layer.thumbDirty = true;
        this._sshadow.set(o.id, this._ssig(layer)); // shadow PRIMA: niente eco
        board.mgr.bump();
        a.planes.invalidate();
        a.ui.svgUI.sync(true);
        a.ui.layersUI.scheduleThumbs();
        return;
      }
      ...
      case 'ldup': {
        ...
        if (copy.kind === 'text') this._tshadow.set(copy.id, this._tsig(copy));
        if (copy.kind === 'svg') this._sshadow.set(copy.id, this._ssig(copy));
        a.planes.invalidate();
        a.ui.layersUI.sync();
        a.ui.layersUI.scheduleThumbs();
        return;
      ...
      case 'ldel': {
        ...
        this._tshadow.delete(o.id);
        this._sshadow.delete(o.id);
        a.planes.invalidate();
        a.ui.layersUI.sync();
        return;
      }
  ...
  // Merge livelli remoto: nuovo raster con id forzato + pixel ricevuti,
  // sostituisce i due livelli sorgente con una entry 'swap' atomica.
  /** @param {any} o @param {number} uid */
  async _applyLayerMerge(o, uid) {
    const a = this.app;
    const raw = await this._decodeBigPayload(o, uid, 'merged layer');
    if (!raw || !this._validChunkPayload(raw, 'merged layer')) return;
    const board = a.boards.byId(o.b);
    const ids = Array.isArray(o.ids) ? o.ids.map((v) => Number(v)) : [];
    if (!board || ids.length !== 2 || ids.some((v) => !Number.isFinite(v))) return;
    const i0 = board.mgr.indexOf(ids[0]);
    const i1 = board.mgr.indexOf(ids[1]);
    if (i0 < 0 || i1 < 0 || Math.abs(i0 - i1) !== 1) return;

    const raster = makeRasterLayer(o.nm, a.heap);
    raster.id = o.id;
    raster.visible = o.vis !== false;
    raster.opacity = 1;
    raster.mode = 'normal';
    if (o.ref) raster.reference = true;
    ...
    const prevAct = board.mgr.activeId;
    const ix = Number.isFinite(o.ix) ? Number(o.ix) : Math.min(i0, i1);
    const activeAfter = ids.includes(prevAct) ? raster.id : prevAct;
    const swapped = a._swapLayerGroup(board.id, ids, [raster], ix, activeAfter);
    if (!swapped) {
      a._destroyOwnedLayer(raster);
      return;
    }
    if (raster.reference) {
      for (const layer of board.mgr.layers) layer.reference = false;
    ...
    if (o.cid) this._tagNewEntry(o.cid);
    for (const id of ids) {
      this._shadow.delete(id);
      this._tshadow.delete(id);
      this._sshadow.delete(id);
      this._clearRemoteTransform(uid, id);
    }
    this._shadow.set(raster.id, this._sig(raster));
    a.planes.invalidate();
    a.ui.textUI.open(false);
    a.ui.svgUI.open(false);
    a.ui.layersUI.sync();
    a.ui.layersUI.scheduleThumbs();
  }
  ...
  // SVG rasterizzato remoto: stesso payload pixel del testo, ma rimuove il
  // piano vettoriale SVG invece delle cache testo.
  /** @param {any} o @param {number} uid */
  async _applySvgRaster(o, uid) {
    const a = this.app;
    const raw = await this._decodeBigPayload(o, uid, 'svg raster');
    if (!raw || !this._validChunkPayload(raw, 'svg raster')) return;
    const board = a.boards.byId(o.b);
    const svg = board && board.mgr.byId(o.sl);
    if (!board || !svg || svg.kind !== 'svg') return;
    const raster = makeRasterLayer(o.nm, a.heap);
    raster.id = o.id;
    raster.visible = svg.visible;
    raster.opacity = svg.opacity;
    a._allStores.add(raster.store);
    const dv = new DataView(raw.buffer, raw.byteOffset, raw.byteLength);
    const n = Math.floor(raw.length / (8 + CHUNK_BYTES));
    for (let i = 0; i < n; i++) {
    ...
    const prevAct = board.mgr.activeId;
    const d = board.mgr.detach(o.sl);
    if (!d) return;
    board.mgr.insert(raster, d.index);
    if (prevAct && prevAct !== o.sl) board.mgr.activeId = prevAct;
    freeSvgPlane(svg);
    a.undoMgr.pushStruct(/** @type {any} */ (
      { op: 'replace', layer: svg, layerId: raster.id, boardId: board.id }));
    if (o.cid) this._tagNewEntry(o.cid);
    this._sshadow.delete(o.sl);
    this._shadow.delete(o.sl);
    this._shadow.set(raster.id, this._sig(raster));
    a.planes.invalidate();
    a.ui.svgUI.open(false);
    a.ui.layersUI.sync();
    ...
    for (const b of a.boards.boards) {
      ...
      for (const l of b.mgr.layers) {
        ...
        if (l.kind === 'raster') {
          ...
          lm.svgItem = structuredClone(l.svgItem);
        }
        layers.push(lm);
      }
      boardsMeta.push({
  ...
  async _applySnapshot() {
    ...
    for (const bm of meta.boards) {
      ...
      for (const lm of bm.layers) {
        ...
        } else if (lm.kind === 'text') {
          layer = makeTextLayer(lm.nm, lm.item, lm.style);
          layer.id = lm.id;
        } else if (lm.kind === 'svg') {
          layer = makeSvgLayer(lm.nm, lm.svgItem);
          layer.id = lm.id;
          layer.svgDirty = true;
        } else {
          layer = makeRasterLayer(lm.nm, a.heap);
  ...
  /** Firma del contenuto di un livello SVG. @param {Layer} l */
  _ssig(l) {
    return JSON.stringify(l.svgItem);
  }
  ...
  _watch(now) {
    ...
    for (const b of this.app.boards.boards) {
      for (const l of b.mgr.layers) {
        ...
        if (l.kind === 'text') {
          ...
          const ssig = this._ssig(l);
          const sold = this._sshadow.get(l.id);
          if (sold === undefined) {
            this._sshadow.set(l.id, ssig);
          } else if (sold !== ssig) {
            this._sshadow.set(l.id, ssig);
            this._sendOp({
              k: 'sset', id: l.id,
              svgItem: structuredClone(l.svgItem),
            });
          }
        }
      }
    ...
    for (const id of this._sshadow.keys()) {
      if (!seen.has(id)) this._sshadow.delete(id);
    }
  }
  ...
  _applyPatches() {
    ...
    swap(a.undoMgr, 'pushStruct', /** @param {any} e */ function (e) {
      ...
      } else if (e.op === 'svgform') {
        if (self._muteSform) return;
        const ncid = self._nextCid();
        self._tagNewEntry(ncid);
        const sl = a.boards.layerById(e.layerId);
        if (sl) self._sshadow.set(e.layerId, self._ssig(sl));
        self._sendOp({ k: 'sform', l: e.layerId, m: e.tm1, cid: ncid });
      }
    });
    ...
    swap(a, 'addLayer', /** @param {Layer} layer @param {any} [board] */ function (layer, board = a.boards.active) {
      ...
      if (ok && !self._mute) {
        ...
        if (layer.kind === 'text') {
          ...
          o.kind = 'svg';
          o.svgItem = structuredClone(layer.svgItem);
          self._sshadow.set(layer.id, self._ssig(layer));
        }
        self._sendOp(o);
      }
      return ok;
    ...
    swap(a, 'importImageLayer', /** @param {File} file */ async function (file) {
      ...
      if (isSvg) return await origImport(file);
    ...
    swap(lui, '_duplicate', function () {
      ...
      if (copy && copy !== src) {
        ...
        if (copy.kind === 'text') self._tshadow.set(copy.id, self._tsig(copy));
        if (copy.kind === 'svg') self._sshadow.set(copy.id, self._ssig(copy));
    ...
    const origSvgRast = a.rasterizeSvgLayer.bind(a);
    swap(a, 'rasterizeSvgLayer', /** @param {number} id */ async function (id) {
      if (locked()) return false;
      const board = a.boards.boardOfLayer(id);
      const ok = await origSvgRast(id);
      if (!ok || !board) return ok;
      const raster = board.mgr.active; // lo scambio l'ha resa attiva
      if (!raster || !raster.store) return ok;
      const cid = self._nextCid();
      ...
      const raw = packCurrent(raster.store, nonBlankChunkList(raster.store));
      self._sendBig({
        k: 'srast', b: board.id, sl: id, id: raster.id, nm: raster.name,
        cid,
      }, raw);
      return ok;
    });

    const origMerge = a.mergeLayerAdjacent.bind(a);
    swap(a, 'mergeLayerAdjacent', /** @param {number} id @param {'above'|'below'} direction */ async function (id, direction) {
      if (locked()) return false;
      const board = a.boards.boardOfLayer(id);
      const mgr = board && board.mgr;
      const idx = mgr ? mgr.indexOf(id) : -1;
      const otherIdx = direction === 'above' ? idx + 1 : idx - 1;
      if (!board || !mgr || idx < 0 || otherIdx < 0 || otherIdx >= mgr.layers.length) {
        return await origMerge(id, direction);
      }
      const lowerIdx = Math.min(idx, otherIdx);
      const ids = [mgr.layers[lowerIdx].id, mgr.layers[lowerIdx + 1].id];
      const ok = await origMerge(id, direction);
      if (!ok) return ok;
      const raster = board.mgr.active;
      if (!raster || !raster.store) return ok;
      const cid = self._nextCid();
      self._tagNewEntry(cid);
      for (const oldId of ids) {
        self._shadow.delete(oldId);
        self._tshadow.delete(oldId);
        self._sshadow.delete(oldId);
      }
      self._shadow.set(raster.id, self._sig(raster));
      const raw = packCurrent(raster.store, nonBlankChunkList(raster.store));
      self._sendBig({
        k: 'lmerge', b: board.id, ids, id: raster.id, nm: raster.name,
        ix: lowerIdx, vis: raster.visible, ref: !!raster.reference, cid,
      }, raw);
      return ok;
    });
    ...
    // tutto il resto dell'app è ormai collaborativo.
  }
