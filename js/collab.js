...
import { makeRasterLayer, makeTextLayer, bumpLayerIds } from './layers.js';
import { makeBrushTexture } from './texture.js';
import { makeBrushShape } from './shape.js';
import { CHUNK_BYTES, chunkKey } from './store.js';
...
/** Chunk senza alcun pixel scritto (premultiplied: parola 0 = vuoto). @param {any} c */
...
export class Collab {
  ...
  _onData(conn, m) {
    ...
    switch (m.t) {
      ...
      case 'op': {
        const uid = host ? (g && g.state === 'live' ? g.uid : -1) : m.u;
        if (uid < 0 || uid === this.uid) return;
        if (host) this._broadcast({ t: 'op', u: uid, o: m.o }, conn);
  ...
  _applyOp(o, uid) {
    ...
    switch (o.k) {
      ...
      case 'tmove': {
        ...
        if (!board) return;
        const lost = a._undoHost().translateLayer(o.l, o.dx, o.dy);
        if (!lost) return;
        this._pendingCid = o.cid || null; // il wrap di pushMove tagga (no echo)
        a.undoMgr.pushMove(/** @type {any} */ (
          { layerId: o.l, dx: o.dx, dy: o.dy, boardId: board.id, chunks: lost }));
        this._pendingCid = null;
        a.planes.invalidate();
      ...
      case 'ldup': {
        ...
        let copy;
        if (src.kind === 'raster') {
          copy = makeRasterLayer(o.nm, a.heap);
          a._allStores.add(copy.store);
          ...
          copy = makeTextLayer(o.nm, { ...src.item }, structuredClone(src.style));
        }
        copy.id = o.id;
  ...
  undoOwn() {
    ...
    this._broadcast({ t: 'op', u: this.uid, o: { k: 'undo', cid } }, null);
    a.ui.updateUndoButtons(a.undoMgr);
  }
  ...
  redoOwn() {
    ...
    this._broadcast({ t: 'op', u: this.uid, o: { k: 'redo', cid } }, null);
    a.ui.updateUndoButtons(a.undoMgr);
  }
      ...
      for (const l of b.mgr.layers) {
        ...
        if (l.kind === 'raster') {
          ...
          for (const c of l.store.map.values()) {
            if (!chunkBlank(c)) chunks.push(c);
          }
          if (chunks.length) {
            const raw = new Uint8Array(chunks.length * (8 + CHUNK_BYTES));
            const dv = new DataView(raw.buffer);
            let off = 0;
            for (const c of chunks) {
              dv.setInt32(off, c.cx, true);
              dv.setInt32(off + 4, c.cy, true);
              raw.set(c.data, off + 8);
              off += 8 + CHUNK_BYTES;
            }
  ...
  _applyPatches() {
    ...
    swap(a.undoMgr, 'pushMove', /** @param {any} e */ function (e) {
      ...
      self._broadcast({
        t: 'op', u: self.uid,
        o: { k: 'tmove', l: e.layerId, dx: e.dx, dy: e.dy, cid: ncid },
      }, null);
    });
    ...
    swap(a.undoMgr, 'pushStruct', /** @param {any} e */ function (e) {
      ...
      self._broadcast({
        t: 'op', u: self.uid,
        o: { k: 'tform', l: e.layerId, x: e.x1, y: e.y1, s: e.s1, cid: ncid },
      }, null);
    });
    ...
    swap(a, 'addLayer', /** @param {Layer} layer @param {any} [board] */ function (layer, board = a.boards.active) {
      ...
      if (ok && !self._mute) {
        ...
        self._broadcast({ t: 'op', u: self.uid, o }, null);
      }
      return ok;
    });
    ...
    swap(a, 'importImageLayer', /** @param {File} file */ async function (file) {
      ...
      for (const c of layer.store.map.values()) {
        if (!chunkBlank(c)) list.push({ cx: c.cx, cy: c.cy });
      }
      const raw = packCurrent(layer.store, list);
      self._sendBig({
    ...
    swap(a, 'deleteLayer', /** @param {number} id */ function (id) {
      ...
      if (board && !board.mgr.byId(id)) {
        ...
        self._broadcast({ t: 'op', u: self.uid, o: { k: 'ldel', id, cid } }, null);
      }
    });
    ...
    swap(a, 'moveLayerUndoable', /** @param {number} f @param {number} t */ function (f, t) {
      ...
      const cid = self._nextCid();
      self._tagNewEntry(cid);
      self._broadcast({ t: 'op', u: self.uid, o: { k: 'lmove', b: a.boards.activeId, f, t, cid } }, null);
    });
    ...
    swap(a, 'addBoard', function () {
      ...
      if (b) {
        self._broadcast({
          t: 'op', u: self.uid,
          o: { k: 'badd', id: b.id, nm: b.name, x: b.x, y: b.y, w: b.w, h: b.h, fl: b.mgr.layers[0].id },
        }, null);
      }
      return b;
    });
    ...
    swap(lui, '_duplicate', function () {
      ...
      if (copy && copy !== src) {
        ...
        self._broadcast({
          t: 'op', u: self.uid,
          o: { k: 'ldup', src: src.id, id: copy.id, nm: copy.name },
        }, null);
      }
    });
    ...
    swap(a, 'rasterizeTextLayer', /** @param {number} id */ function (id) {
      ...
      /** @type {{cx: number, cy: number}[]} */
      const list = [];
      for (const c of raster.store.map.values()) {
        if (!chunkBlank(c)) list.push({ cx: c.cx, cy: c.cy });
      }
      const raw = packCurrent(raster.store, list);
      self._sendBig({
    ...
    swap(a, 'clearAll', function () {
      ...
      self._broadcast({
        t: 'op', u: self.uid,
        o: { k: 'clear', b: board.id, bnm: board.name, l: first.id },
      }, null);
    });
