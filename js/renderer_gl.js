...
// minima (in px, ±uR) alla forma (alpha>0.5, canale R) e al suo complemento
...
const int MAX_R = ${FX_MAX_R};
void main() {
  bool inside = texture2D(uTex, vUv).a > 0.5;
  ...
  for (int i = 1; i <= MAX_R; i++) {
    ...
    float fi = float(i);
    bool up = texture2D(uTex, vUv - uStep * fi).a > 0.5;
    bool dn = texture2D(uTex, vUv + uStep * fi).a > 0.5;
...
void main() {
  ...
  // con alpha > 0.5 contano "dentro" ma il loro residuo 1-a va riempito,
 ...
 * @property {number} id timbro di sessione: cambia = texture da ricostruire
 * @property {'gauss'|'motion'|'zoom'|'noise'|'grain'|'thresh'|'halftone'|'stroke'|'tint'} kind
 * @property {number} layerId
 * @property {import('./store.js').ChunkStore} store
 * @property {number} x @property {number} y origine mondo della texture
 * @property {number} w @property {number} h
...
export class GLRenderer {
   ...
   * textQuads: i livelli testo NON in editing stanno nella pila come gli
  ...
  render(camera, layers, activeId, strokeStore, strokeOpacity, eraserLive, proxies = null, transform = null, fx = null, textQuads = null) {
    ...
    for (let i = 0; i < layers.length; i++) {
      ...
      const inSession = (transform !== null && transform.layerId === layer.id) ||
  ...
  _drawTextQuad(camera, layer, q) {
    ...
    // il blending del livello è sempre normal (i testi non hanno mode)
    gl.blendFunc(gl.ONE, gl.ONE_MINUS_SRC_ALPHA);
  ...
  _renderClipGroup(camera, layers, baseIdx, endIdx, activeId, strokeStore, strokeOpacity, eraserLive, transform, fx, cx0, cy0, cx1, cy1) {
    ...
    const inSession = (transform !== null && transform.layerId === base.id) ||
    ...
    if (SHADER_MODES.has(mode)) {
      ...
      const rect = inSession
        ? this._clipRectDev(camera, transform !== null && transform.layerId === base.id
