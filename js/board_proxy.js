// PROXY DEI BOARD — cache di presentazione degli artboard NON attivi.
...
// shader dei chunk. Risultato nel caso reale "fit su un artboard e disegno":
// solo l'artboard attivo resta live; gli altri passano da O(chunk × layer) a
// O(board) e le texture dei chunk vengono liberate dalla GPU. La CPU resta la
// verità: il proxy è solo presentazione e si ricostruisce — a budget, un
// board per volta — quando cambiano struttura, visibilità/opacità o pixel.
//
// Transizioni:
// - non attivo: coperto subito dal proxy; se il proxy non è ancora pronto il
//   board può restare bianco per qualche frame, ma non entra nel path live.
// - quando un board diventa attivo: il quad resta a coprire finché i chunk
//   visibili non sono stati ricaricati (WARM_BUDGET a frame), poi torna live.
...
// proxy è comunque solo per board non attivi, quindi privilegia fluidità e
...
export class BoardProxyCache {
  ...
  /**
   * Diagnostica per la console performance: fotografa quanti board sono live
   * e quanti sono coperti da proxy nel frame corrente.
   * @param {BoardManager} boards
   * @param {Camera} camera
   * @param {number} activeBoardId
   */
  stats(boards, camera, activeBoardId) {
    ...
    return {
      ...
      loadingProxies,
      proxyTextures,
      proxyBytes: proxyTextures * PROXY_SIZE * PROXY_SIZE * 4,
