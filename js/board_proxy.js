// PROXY DEI BOARD — cache di presentazione per gli artboard non attivi.
// Ogni board non attivo si disegna come UN quad con una texture piatta 1024²:
// il composito dei suoi livelli (testo compreso, rasterizzato alla sua
// posizione nella pila), fatto in GPU dentro un framebuffer con lo stesso
// shader dei chunk. Risultato: draw call da O(chunk × layer) a O(board),
// VRAM residente da centinaia di MB a ~5 MB per board, e niente upload
// massivo per artboard fuori focus. La CPU resta la verità: il proxy è solo
// presentazione e si ricostruisce — a budget, un board per volta — quando
// cambiano struttura (epoch), visibilità/opacità o pixel (store.ver).
//
// Transizioni:
// - board inattivo senza proxy pronto: i layer sono comunque nascosti subito;
//   se non c'è ancora una texture disegnabile resta bianco per qualche frame,
//   invece di riaccendere tutto il path per-chunk.
// - selezione del board: il quad resta a coprire finché i chunk visibili non
//   sono stati ricaricati (WARM_BUDGET a frame), poi il board attivo torna al
//   path per-chunk nitido.
...
// 1024² da un board 2048² è una riduzione esatta 2:1: i board inattivi sono
// preview leggere anche a zoom alto; la qualità piena torna quando il board
// viene selezionato.
const PROXY_SIZE = 1024;
const BUILD_BUDGET = 16;   // default idle: chunk-layer compositati per frame
...
export class BoardProxyCache {
  constructor() {
    ...
    this._fbo = null;
    // build incrementale: un board per volta, con budget dinamico a frame.
    // grpEnd/grpBase: gruppo di ritaglio in corso (-1 = nessuno),
   ...
   * @param {Camera} camera
   * @param {number} buildBudget chunk-layer da comporre in questo frame
   * @param {number} buildMaxMs tempo massimo da dedicare al build in questo frame
   * @param {Planes|null} [planes] per svuotare le cache 2D dei board coperti
   * @returns {ProxyFrame}
   */
  update(renderer, boards, activeBoardId, camera, buildBudget = BUILD_BUDGET, buildMaxMs = 2, planes = null) {
    const out = this._out;
    out.quads.length = 0;
    out.skip.clear();
    out.loading.clear();
    ...
    for (const b of boards.boards) {
      ...
      // I board non attivi restano sempre coperti dal proxy. Se la texture non
      // e' ancora pronta, si salta comunque il path layer/chunk: meglio un
      // breve placeholder che pagare tutti gli artboard fuori focus.
      if (!e.covering) {
        e.covering = true;
        e.warmDone = 0;
    ...
    if (buildBudget > 0 && buildMaxMs > 0) {
      this._buildTick(renderer, boards, candidate, buildBudget, buildMaxMs);
    }
    return out;
  }
  ...
  // Un passo della build: composita fino a buildBudget chunk-layer nel
   ...
   * @param {number} buildBudget
   * @param {number} buildMaxMs
   */
  _buildTick(renderer, boards, candidate, buildBudget = BUILD_BUDGET, buildMaxMs = 2) {
    let bld = this._build;
    ...
    let budget = Math.max(0, buildBudget | 0);
    while (budget > 0 && performance.now() - t0 < buildMaxMs) {
