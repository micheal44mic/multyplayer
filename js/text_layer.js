...
/** @param {CanvasRenderingContext2D} ctx @param {Path2D} path @param {TextStyle} st @param {number} x0 @param {number} y0 @param {number} alpha */
function drawPathShadow(ctx, path, st, x0, y0, alpha) {
  ...
  if (!st.block && st.shadowBlur > 0) {
    ...
    ctx.shadowOffsetX = ux * st.shadowDist;
    ctx.shadowOffsetY = uy * st.shadowDist;
...
/** @param {CanvasRenderingContext2D} ctx @param {TextItem} it @param {TextStyle} st @param {number} x0 @param {number} y0 @param {number} alpha */
function drawVectorDistortDocument(ctx, it, st, x0, y0, alpha) {
  const d = textDistortPathData(it, st);
  if (d === null || typeof Path2D !== 'function') return false;
  const path = new Path2D(d);
  drawPathShadow(ctx, path, st, x0, y0, alpha);
...
export function drawTextDocument(ctx, it, st, x0, y0, alpha = 1) {
  if (distortOn(st)) {
    drawVectorDistortDocument(ctx, it, st, x0, y0, alpha);
    return;
  }
  ctx.globalAlpha = alpha;
  ctx.font = textFont(it, st);
